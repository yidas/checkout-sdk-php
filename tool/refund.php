...
$refundRequest->amount = (isset($_GET['amount']) && $_GET['amount']!="") ? (integer) $_GET['amount'] : null;
$refundRequest->reference = ($order['params']['reference']) ? $order['params']['reference'] . "R" . (string) substr(round(microtime(true) * 1000), -3) : null;

try {

    $response = $paymentsClient->refundPayment($transactionId, $refundRequest);
